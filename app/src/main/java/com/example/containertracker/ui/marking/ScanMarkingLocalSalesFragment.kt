...
import com.example.containertracker.databinding.FragmentScanMarkingLocalSalesBinding
...
class ScanMarkingLocalSalesFragment: BaseFragment(), ZXingScannerView.ResultHandler {

    companion object {
        private var isProgressInput = false
    }

    private val viewModel: ScanMarkingLocalSalesViewModel by viewModel()
    lateinit var binding: FragmentScanMarkingLocalSalesBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentScanMarkingLocalSalesBinding.inflate(inflater, container, false)
    ...
    private fun mainView() {
        binding.apply {
            ...
            buttonSubmitContainer.setOnClickListener {
                ...
                viewModel.scanMarkingLocalSales(flag = FlagScanEnum.INPUT)
            }
            initScanner()
        }
    }
...
//                handleStatusSaved(it)
    ...
    override fun handleResult(result: Result?) {
        ...
        if (resultString.isNotBlank()) {
            ...
            pauseCamera()
            viewModel.scanMarkingLocalSales(resultString, FlagScanEnum.SCAN)
        }
    }

    private fun resumeCamera() = lifecycleScope.launch(Dispatchers.Main) {
        scannerView?.setResultHandler(this@ScanMarkingLocalSalesFragment)
