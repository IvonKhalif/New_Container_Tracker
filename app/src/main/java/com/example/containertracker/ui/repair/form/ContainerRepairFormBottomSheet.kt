package com.example.containertracker.ui.repair.form
...
import com.example.containertracker.databinding.BottomSheetContainerRepairFormBinding
...
class ContainerRepairFormBottomSheet : BaseBottomSheet() {

    private lateinit var binding: BottomSheetContainerRepairFormBinding
    private val viewModel: ContainerRepairFormViewModel by viewModel()
    ...
    var onSubmitClicked: (List<GenericSelectImageUiModel>) -> Unit = {}
    ...
    override fun onCreateView(
        ...
        binding = DataBindingUtil.inflate(
            inflater,
            R.layout.bottom_sheet_container_repair_form,
            container,
            false
        )
    ...
    private fun mainView() {
        viewModel.apply {
            openMediaState.observe(viewLifecycleOwner) { imageModel ->
                showMediaDialog(data = imageModel)
            }
            openImagePreviewState.observe(viewLifecycleOwner) { imageModel ->
        ...
        with(binding) {
            ...
            buttonSubmit.setOnClickListener {
                onSubmitClicked(viewModel.imageList.value.orEmpty())
            }
    ...
    private fun showMediaDialog(data: GenericSelectImageUiModel) {
        showImmediately(childFragmentManager, "show_media") {
            SelectImageBottomSheet.newInstance().apply {
                onImageSelected = {
                    this@ContainerRepairFormBottomSheet.compressImage(data, PickImageUtils.uri)
                }
            }
        }
    }
    ...
    companion object {
        fun newInstance(): ContainerRepairFormBottomSheet {
            val sheet = ContainerRepairFormBottomSheet()
